...
function getCfgFromRedis(key, callback) {
    "use strict";
    var ret = null;
...
getCfgFromRedis(redisKeys.dpaConfig, function (val) {
    "use strict";
    cfg = val;
    winston.info('info', 'Loaded config values from redis:');
});
...
server.route({
    ...
    handler: function (request, reply) {
        ...
        updateCfg(data.key, data.value);

        //cfg[key] = cnt;

        saveData2Redis(redisKeys.dpaConfig, JSON.stringify(cfg));
...
server.route({
    method: 'GET',
    path: '/get/configuration/{cfgdata}',
    handler: function (request, reply) {
        "use strict";
        var d = request.params.cfgdata,
            data = JSON.parse(d),
            key = data.key,
            val = getCfg(key);
        console.log("Sending data for", data.key, val);
        reply(val);
    }
});
